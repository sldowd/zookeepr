...
function filterByQuery(query, zookeepers) {
    ...
    if (query.age) {
        filteredResults = filteredResults.filter(
            // since our form data will be coming in as strings 
            // and our JSON is storing as a number, we must convert 
            // the query string to a number to form a comparison:
...
function findById(id, zookeepers) {
    const result = zookepers.filter((zookeeper) => zookeeper.id === id)[0];
    return result;
};
